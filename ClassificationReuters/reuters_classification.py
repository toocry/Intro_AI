...
def create_testing_data():
    ...
    # Considering only the 10000 most often used words, vectorization produces a vector of length 10000, index i = 1 if word i is in the newswire
...
def categorize_testing_set(input, model):
...
def my_evaluate(input, model):
    (train,train_labels,_,_) = input
    print("Train: ")
    model.evaluate(train[1000:], train_labels[1000:])
    print("Validation: ")
    model.evaluate(train[:1000], train_labels[:1000])
...
if __name__ == "__main__":
    ...
    #my_evaluate(input, model)
    categorize_testing_set(input, model)
